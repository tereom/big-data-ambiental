...
head(insurance)
insurance_dat <- select(insurance, Age, GoodStudent, SocioEcon,
    RiskAversion, Accident, DrivQuality)
blacklist <- data.frame(from = c("DrivQuality", "Accident"),
    to = c("Age", "DrivQuality"))
# write_csv(insurance_dat, "data/insurance_dat.csv")
```
...
insurance_fit <- bn.fit(insurance_gm, data = insurance_dat, 
                        method = 'bayes', iss = 1)
#write.net(file = './salidas/insurance.net', insurance_fit)
```
...
black <- data.frame(
  from = c('drenaje', 'nivelaprob', 'decil', 'nivelaprob'),
...
![](imagenes/manicule2.jpg) Exporta la red a SAMIAM y compara con la red que usó MLE.
