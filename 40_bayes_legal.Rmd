...
P(E|Culpable)
$$
Así como la probabilidad de que se produzca la evidencia E cuando el sospechoso no es culpable.
...
## Más allá de una duda razonable: _in dubio pro reo
...
o_evidencia <- p_evidencia_culpable / p_evidencia_inocente
o_evidencia
```
...
Por lo tanto, el presunto asesino tiene un momio de `r o_evidencia_culpable` en su contra.

Como un momio es una razón de probabilidades, entonces
$$
O=\frac{P(A)}{P(no A)}=\frac{P(A)}{1-P(A)} \implies P(A)=\frac{O}{1+O}
$$ 
lo que permite calcular la probabilidad de ser culpable a la luz de la evidencia, en este caso: `r format(o_evidencia_culpable/(1+o_evidencia_culpable) * 100, digits=4)`%. ¿Qué cabría decidir en el juicio?
...
o_evidencia_culpable
```
En este otro escenario el presunto asesino tiene un momio de `r o_evidencia_culpable` en su contra y por lo tanto, la nueva probabilidad de ser culpable a la luz de la evidencia es: `r format(o_evidencia_culpable/(1+o_evidencia_culpable) * 100, digits=4)` %. ¿Qué cabría decidir ahora en el juicio?
