...
muestra  <- params$n_muestra
dulces <- params$n_limon
...
ggplot(posterior_hi_dj_long, aes(x = n, y = p, colour = bolsa)) + 
    geom_line() + geom_point() + ylim(c(0,1))
...
```{r, echo=FALSE}
a <- bayes_dulces(params$n_muestra, params$n_limon, priors, dulces_limon_hi)
kable(tibble(bolsa=c("b1", "b2", "b3", "b4", "b5"), prior=a), digits = 6) %>%
